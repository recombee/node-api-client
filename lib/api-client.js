...
class ApiClient {
  ...
  send(request, callback) {
    ...
    let options = {
        ...
        headers: {'Accept': 'application/json',
                  ...
                  'User-Agent': 'recombee-node-api-client/5.0.2.debug-timings'},
            ...
            .catch((error) => {
              if (error instanceof got.HTTPError) {
                ...
                if(error.code === 'ETIMEDOUT' || error.code === 'ESOCKETTIMEDOUT') {
                  if (error.timings?.phases) {
                    delete error.timings.phases; // got doesn't calculate them correctly
                  }
  ...
  _encodeRequestQueryParams(request) {
    ...
    queryParams["debugId"] = crypto.randomUUID();
    let paramPairs = [];
