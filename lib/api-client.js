...
class ApiClient {
  ...
  constructor(databaseId, token, options) {
    ...
    if (Object.getPrototypeOf(this.options) !== Object.prototype)
      throw new Error(
        `options must be given as an Object (${this.options} given instead)`
      );
  ...
  send(request, callback) {
    if (
      request instanceof requests.Batch &&
      request.requests.length > BATCH_MAX_SIZE
    )
    ...
    const agent =
      request.ensureHttps && this.options.agents.https
        ? { https: this.options.agents.https }
        : this.options.agents.http
        ? { http: this.options.agents.http }
        : undefined;
    console.log("agtent", request.ensureHttps, agent);
  ...
  _getBaseUri() {
    ...
    if (this.options.region) {
      if (baseUri) {
        throw new Error(
          "baseUri and region cannot be specified at the same time"
        );
      }
  ...
  _encodeRequestQueryParams(request) {
    ...
    for (let d in queryParams)
      paramPairs.push(
        this._rfc3986EncodeURIComponent(d) +
          "=" +
          this._formatQueryParameterValue(queryParams[d])
      );
    res += paramPairs.join("&");
    if (res.length > 0) {
      res = "?" + res;
  ...
  _formatQueryParameterValue(value) {
    if (value instanceof Array) {
      return value
        .map((v) => this._rfc3986EncodeURIComponent(v.toString()))
        .join(",");
    }
  ...
  _send_batch_part_rec(requests, results) {
    ...
    return this.send(request).then((result) => {
      results.push(result);
